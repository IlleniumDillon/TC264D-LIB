...
int core0_main(void)
{
    ...
    while(1)
    {
        static uint8 i[5] = {0xa5,0xa6,0xa7,0xa8,0xa9};
        uint8 j[5];

        //Hsic_Log_Write(HSIC_LOG_VERBOSE,"CPU0","log\r\n");
        //Hsic_swIICWrite(&dev,0x7e,&i,1);
        //Hsic_hwSPIExchange(&spi,&i,NULL,1);
        //Hsic_DelayMS(STM0,1);
        //Hsic_hwSPIExchange(&spi,NULL,NULL,10);

        //Hsic_hwSPIExchange(&spi,&i,NULL,1);

        //Hsic_swSPIExchange(&dev,&i[0],&j[0],5);
        Hsic_encDataUpdate(&data,IfxGpt120_T2INA_P00_7_IN);
        HSIC_LOGE("CPU0","Position:%d speed:%f\r\n",data.rawPosition,data.speed);
        Hsic_DelayMS(STM0,5);
    }
    return (1);
}
